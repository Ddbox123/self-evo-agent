...
class PromptManager:
    ...
    def __init__(self):
        ...
        # 从 config.toml 读取默认组件列表
        self._default_components = self._load_default_components_from_config()
    ...
    def _scan_workspace_prompts(self):
        ...
        if self._dynamic_root.exists():
            for md_file in sorted(self._dynamic_root.glob("*.md")):
                if md_file.stem.upper() == "CODEBASE_MAP":
                    continue  # CODEBASE_MAP 由 no-file 组件动态生成，不走静态文件加载
    ...
    def _register_no_file_components(self):
        ...
        no_file_components = [
            ...
            ("CODEBASE_MAP",   30, False, self._load_codebase_map, "基于 AST 扫描的代码库结构认知地图（每次 build 动态更新）"),
    ...
    def _load_default_components_from_config(self) -> List[str]:
        """从 config.toml 读取 [prompt].default_components，读取失败时 fallback 到内置默认值"""
        try:
            from config import get_config
            components = get_config().prompt.default_components
            if components:
                from core.logging import debug_logger; debug_logger.info(f"[PromptManager] 从 config 加载默认组件: {components}")
                return components
        except Exception:
            pass
    ...
    def _select_components(
        ...
        if include is not None:
            ...
            effective_include = self._default_components
