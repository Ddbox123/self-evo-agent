...
├── decision/           决策树、优先级、策略、分类
├── infrastructure/     工具执行、注册、状态、事件、安全
├── learning/           学习引擎、反馈、洞察、策略学习
├── logging/            记录器、转录、追踪
...
> INDEX.md 引用但磁盘不存在的目录（evolution/knowledge/autonomous/ecosystem/capabilities）视为历史规划遗留。Agent 发现磁盘与 INDEX.md 不符时，按第3节"状态不同步"处理。
